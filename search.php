<?php $connect = mysqli_connect("localhost","root","","travello"); ob_start()?>
						...
						<div class="home_search_content">
							<form action="#" class="home_search_form" id="home_search_form">
								<div class="d-flex flex-lg-row flex-column align-items-start justify-content-lg-between justify-content-start">
									<select name="" class="search_input search_input_1" placeholder="City" required="required"  id="">
									...
									<input type="text" class="search_input search_input_4" placeholder="Budget in Naira" required="required">
		...
		<div class="container">
			<div class="row">
				<div class="col text-center">
                    ...
                    <div class="section_title"><h2>Search results for :</h2></div>
                    <div class="home_search_content">
							<form action="" method="post" class="home_search_form" id="home_search_form">
								<div class="d-flex flex-lg-row flex-column align-items-start justify-content-lg-between justify-content-start">
									<select name="city_name" value="<?php echo $_POST["city_name"];?>" class="search_input search_input_1" placeholder="City" required="required"  id="">
									...
									<input value="<?php echo $_POST["departure"];?>" type="text" name="departure" class="search_input search_input_2" placeholder="Departure">
									<select name="arrival" class="search_input search_input_3" required placeholder="Arrival Month">
									...
									<input value="<?php echo $_POST["price"];?>" type="number" name="price" class="search_input search_input_4" placeholder="Budget in Naira" required="required">
									<button type="submit" name="search" style="border:none; cursor:pointer;" class="text-white btn btn-lg signup"><i class="fa fa-search"></i> Search Again</button>
								</div>
							</form>
						</div>
				</div>
					...
					<div class="destinations_container item_grid">
						<?php
							if(!isset($_POST["search"])){
								...
								$price = $_POST["price"];
								$search_query = mysqli_query($connect,"SELECT * FROM tour_places WHERE state = '$name' AND (price = $price OR price < $price)");
								if(mysqli_num_rows($search_query) == 0 ){
									echo "<div class='alert alert-info col-12 text-center h4'>Sorry, there is no tourism destination matching your search entry. Please try again or send a message if you need a special tour.</div>";
									echo "<div class='col-12 text-center'><a href='contact.php' class='login btn btn-md text-white'><i class='fa fa-envelope'></i> Send us a message</a></div>";
								}else{
									$search_month_query = mysqli_query($connect,"SELECT * FROM tour_places WHERE state = '$name' AND (price = $price OR price < $price) AND months LIKE '%$arrival%'");
									if (mysqli_num_rows($search_month_query) == 0){
								while($tour = mysqli_fetch_assoc($search_query)){
									$tour_name = $tour["name"];
									$tour_id = $tour["id"];
									...
									$GLOBALS["msg"] = "<div class='alert alert-success'>Sorry we don't actually go to any tour destination in $name during $arrival but this is the list of other $name state destinations we tour on  other months</div>";
							...
							<div class="destination item" style="margin: 2rem 0;">
								...
								<div class="destination_content">
								 ...
								 <a href="place.php?place=<?php echo $tour_id;?>" class="btn btn-success btn-sm">See more <i class="fa fa-chevron-right"></i></a>
								</div>
							</div>
								<?php
							}
							}else{
								while($tour_month = mysqli_fetch_assoc($search_month_query)){
									$tour_month_name = $tour_month["name"];
									$tour_month_id = $tour_month["id"];
									$tour_month_locality = $tour_month["locality"];
									$tour_month_state = $tour_month["state"];
									$tour_month_image = $tour_month["image_main"];
									$tour_month_price = $tour_month["price"];
									$tour_month_months = $tour_month["months"];
									$GLOBALS["msg"] = "";
								?>
							...
							<div class="destination item" style="margin: 2rem 0;">
								<div class="destination_image">
									<img src="images/<?php echo $tour_month_image;?>" alt="">
								</div>
								<div class="destination_content">
									<div class="destination_title"><a href="destinations.html" style="font-size: 1.2rem;"><?php echo $tour_month_name ." - " .$tour_month_locality;?></a></div>
									<div class="destination_subtitle"><p><?php echo $tour_month_state;?></p></div>
									<div class="destination_price" style="margin:-1px 0 9px 0;">From N<?php echo $tour_month_price;?></div>
									<div class="months" style="margin-bottom: 2rem;"><h4>Months:</h4><p><?php echo $tour_month_months;?></p></div>
								 <a href="place.php?place=<?php echo $tour_month_id;?>" class="btn btn-success btn-sm">See more <i class="fa fa-chevron-right"></i></a>
								</div>
							</div>
